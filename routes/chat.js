...
router.get('/chat', function(req, res, next){
    res.render('admin/admin-chat', {title: 'Chat'});
})

var onlineUserCount = 0; //客户端连接数量
var onlineUsers = {}; //统计客户端登陆用户

router.prepareServer = function(server){
    io.listen(server);
    io.on('connection', function(socket){
        ...
        //当服务端收到消息后，再把该消息播放出去，继续出发chat message事件，然后在客户端监听chat message事件
        socket.on('chat message', function(msg){
            console.log('chat message' + msg);
            var obj = {time: getTime()}; // 构建客户端返回的对象

            //判断是不是第一次链接，以第一条消息作为昵称
            if(!client.name){
                onlineUserCount++;
                ...
                //如果不是第一次聊天，则返回正常的聊天信息
                obj['text'] = msg;
                obj['author'] = client.name;
                obj['type'] = 'message';
                console.log(client.name + ' say:' + msg);

                socket.emit('chat message', obj);//发送给自己的消息，如果不想打印自己发送的消息则注释此句
        ...
        socket.on('disconnect', function(){
            ...
            console.log(client.name + 'disconnect, 在线人数：' + onlineUserCount);
        });
    });
}
